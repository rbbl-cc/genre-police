...
fun main() {
    gitlabCi(validate = true, "../.gitlab-ci-generated.yml") {
        ...
        val dockerBuildJob = podmanBuildJob(
            "docker-build", gitlabCiSource, dockerFile = "./app/Dockerfile"
        ) {
            stage = Stages.Build
            rules {
                +Rules.dev
        ...
        podmanMoveJob(
            "docker-publish-dev", gitlabCiSource, Targets.DockerHubDev
        ) {
        ...
        podmanMoveJob(
            "docker-publish-master", gitlabCiSource, Targets.DockerHubLatest
        ) {
            needs(dockerBuildJob)
            stage = Stages.Publish
            rules {
        ...
        podmanMoveJob(
            "docker-publish-release-candidate", gitlabCiSource, Targets.DockerHubTagged
        ) {
            needs(dockerBuildJob)
            stage = Stages.Publish
            rules {
        ...
        podmanMoveJob("docker-publish-release", gitlabCiSource, Targets.DockerHubTagged) {
            needs(dockerBuildJob)
            stage = Stages.Publish
            rules {
                +Rules.release
