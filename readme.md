...
```shell
java -jar genre-police-1.0.3.jar DiscordToken=token SpotifyClientId=id SpotifyClientSecret=secret
``` 
...
example:
```shell
docker run -d --name gp_1.0.3 registry.gitlab.com/rbbl/genre-police:latest DiscordToken=token SpotifyClientId=id SpotifyClientSecret=secret
```
...
```shell
mvn install
java -jar target/genre-police-1.0.3.jar DiscordToken=token SpotifyClientId=id SpotifyClientSecret=secret
```